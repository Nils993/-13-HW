...
const contant = document.querySelector(".product_contant");
const btnShowCards = document.querySelector(".product_button");
let COUNT_SHOW_CARDS_CLICK = 6;
let shownCards = COUNT_SHOW_CARDS_CLICK;
let countClickBtnShowCards = 1;
let productsData = [];

function response1() {
  if (window.innerWidth <= 781 && window.innerWidth > 530) {
...
async function getProducts() {
  try {
    ...
    if (!res.ok) {
      throw new Error("Ошибка при обработке data.json");
    }
    productsData = await res.json();
    // console.log(productsData);
    if (
      productsData.length > COUNT_SHOW_CARDS_CLICK &&
      btnShowCards.classList.contains("hidden")
    ) {
...
function sliceArrCards() {
  ...
  createCards(arrCards);
  shownCards = contant.children.length;
...
function createCards(data) {
  data.forEach(
    ({ img, id, title, price, discription, color, size, quantity }) => {
      const container = ` 
    
      <div  class="product_item " id="${id}" > 
      <img class="product__img" src=${img} alt="" />
      <div class="product_info">
      <div class="hidden"> <p class="color">Color:<span class="color_name">${color}</span></p>
      <p class="size">Size:<span class="size_value">${size}</span></p>
      <div class="quantity">
        <label for="quantity_num"> Quantity: </label
        ><input value="${quantity}" type="text" id="quantity_num" />
      </div></div>
 ...
 `;
      contant.insertAdjacentHTML("beforeend", container);
    }
  );
}
